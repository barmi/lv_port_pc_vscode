...
void setup() {
    ...
    // 1. 모든 화면을 미리 생성 (객체 주소 할당)
    // 주의: 이벤트 연결을 위해 생성 순서가 중요할 수 있으나, 
    // 전역 포인터를 사용하므로 생성만 되어 있다면 연결은 문제없음
    build_settings_screen();
    build_wifi_screen();
    build_chart_screen();
    build_info_screen();
    build_main_screen(); // 메인 화면은 위 화면 포인터들을 참조하므로 마지막에
